...
import nlu.petshopproject.dao.ReviewDao;
import nlu.petshopproject.jdbc.connectDB;
import nlu.petshopproject.model.Review;

public class ReviewDaoImpl extends connectDB implements ReviewDao {

	@Override
	public void insert(Review review) {
		String sql = "INSERT INTO review(petId,  name, email, content, created) VALUES (?, ?, ?, ?, ?)";
		new connectDB();
		Connection con = connectDB.getConnect();

		try {
			PreparedStatement ps = con.prepareStatement(sql);
			ps.setString(1, review.getPetId());
	...
	public List<Review> getAll() {
		...
		try {
			...
			while(rs.next())
			{
				...
				review.setPetId(rs.getString("petId"));
	...
	{
		List<Review> reviews = new ArrayList<Review>();
		String sql = "SELECT * FROM review WHERE pet_id=?";
		Connection con = connectDB.getConnect();
		try {
			PreparedStatement ps = con.prepareStatement(sql);
			ps.setInt(1, id);
